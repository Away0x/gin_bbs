...
func TopicReplies(topicID int) ([]*Reply, error) {
	...
	if err = database.DB.Where("topic_id = ?", t.ID).Find(&rs).Error; err != nil {
		return rs, err
	}
...
func UserReplies(userID int) ([]*Reply, error) {
	rs := make([]*Reply, 0)
	...
	if err = database.DB.Where("user_id = ?", u.ID).Find(&rs).Error; err != nil {
		return rs, err
	}
